...
Gather only enough evidence to resolve the plan. Treat each required evidence result as a hard dependency barrier: the dependent portion of the plan must not be decided or finalized until that result has returned and the Orchestrator has reconciled it. Independent evidence lanes may run in parallel under Full-style dependency rules. The Orchestrator then returns one actionable plan covering behavior, approach, scope/ownership, sequencing, validation criteria, material risks, migration/rollback concerns when relevant, and unresolved decisions only where evidence cannot resolve them.

All evidence dispatch follows the managed policy's V2-only minimum-context contract. Use native MultiAgent V2 `spawn_agent`; explicitly select the configured Luna role with `agent_type`, use `fork_turns: "none"`, and use concise `explorer_`/`librarian_`-prefixed `task_name` routing metadata. The resulting worker must resolve to GPT-5.6 Luna. Coordinate it only through native V2 `send_message`/`followup_task`; never use generic `send_message_to_thread` or non-V2 agent tools as fallbacks. If V2 or configured-role selection/communication cannot be honored, fail visibly. Keep assignments bounded, self-contained, and no-subagents.
